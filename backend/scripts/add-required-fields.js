...
const requiresId = [
  'audit_logs', 'notifications', 'token_wallets', 'token_transactions',
  'eth_activity', 'crypto_withdrawals', 'transactions', 'ai_suggestions',
  'admin_transfers', 'system_status', 'support_tickets'
];
  ...
  requiresId.forEach(model => {
    ...
    fixed = fixed.replace(createRegex, (match, before, indent, fields, after) => {
      ...
      if (!/\bid:\s*/.test(match)) {
        ...
        const crypto Import = `(await import("crypto")).randomUUID?.() || \`\${Date.now()}\``;
...
function main() {
  ...
  const filesToProcess = tsFiles.filter(f => 
    f.includes('agents') || 
    f.includes('routes') || 
    f.includes('ai-core') ||
    f.includes('services')
