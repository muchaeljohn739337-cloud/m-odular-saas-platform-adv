  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (failureRate > 0.3) {
        ...
        riskScore += 1;
        factors.push(
          `Moderate failure rate: ${(failureRate * 100).toFixed(0)}%`
        );
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        success: true,
        message:
          "Rollback initiated - check Render dashboard to redeploy previous version",
        instructions: [
    ...
    try {
      ...
      res.json({
        ...
        insights: {
          ...
          rollbackCount,
          successRate: `${(
            (successCount / Math.max(deployments.length, 1)) *
            100
          ).toFixed(1)}%`,
